...
### 3.4 踩过的坑（这部分可能对读者最有用）

1. **NPU 只支持静态 Shape**。动态 Shape 模型在 NPU 上编译必然失败——这是预期行为，应记为 `UNSUPPORTED` 而非 `FAIL`，Smoke Test 必须为 NPU 选用静态输入模型。
2. **NPU 首次编译非常慢**（可能数十秒），编译超时必须与 CPU/GPU 分开设置（建议 ≥ 120 s），否则会把正常编译误判为失败。
3. **设备名子串误匹配**：早期版本用子串匹配判断 NPU，结果把 `"Input"` 误判成 NPU。设备识别必须用精确语义，不能偷懒用 `in`。（V0.3 已修复）
4. **依赖缺失不是错误**：full 模式启动前做依赖预检，缺 `openvino-genai` 时 GenAI 环节记 `SKIPPED` 并给出安装指引，其余体检照常完成——而不是整个崩掉。
5. **非目标平台优雅降级**：macOS / 非 Intel 平台只输出环境说明、**不输出 Readiness Score**——一个误导性的低分比没有分数更糟糕。同理，quick 模式没做实测也不出分。
6. **安装决策权归用户**：检测到 OpenVINO 缺失时，Skill 输出引导步骤并以非零码退出，**绝不自动安装软件**。Agent 可以在用户同意后代为执行，但决策必须来自用户。

### 3.5 输出：一份可以直接发给老板的报告
