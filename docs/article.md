...
当前 MVP 采用纯规则引擎实现，已经满足基本需求。后续版本可以接入 OpenVINO 实现：

- **NPU 加速文本解析** — 大文件快速预处理
- **OCR 敏感信息识别** — 扫描图片、截图、扫描版 PDF 中的文字
- **模型辅助语义判断** — 用 35B 小模型辅助识别上下文相关的敏感信息（如"客户名称"在非合同场景下可能不敏感）

这是 AI PC 从"硬件概念"走向"生产力工具"的典型路径：**先用规则解决 80% 的确定性问题，再用模型解决 20% 的模糊性问题。**
