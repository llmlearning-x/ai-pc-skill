...
**OpenVINO 的职责 — 本地模型推理（Agent 大脑）：**

- **意图识别** — 理解用户的自然语言请求（如"检查这个合同能不能外发"）
- **任务规划** — 判断需要调用哪些工具、按什么顺序执行
- **工具调用** — 通过 Function Calling 调用 `scan_privacy_file` 等本地工具
...
| **V0.3** | 多文件批量扫描 | 规则批量执行，模型统一总结 |
| **V0.4** | 单文件夹递归扫描 | 规则批量执行，模型统一总结 |
