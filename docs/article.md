...
- **文件不出电脑** — 原始数据永远不离开设备
- **NPU/GPU 加速** — OpenVINO 可加速本地 OCR 推理和 LLM 模型推理
...
└── demo_id_card.png          # 图片 OCR 场景 (V0.3)
```
...
本项目采用**"模型决策 + 规则执行"**的混合架构，OpenVINO™ 负责模型推理层，规则引擎负责检测执行层，两者各司其职：

**OpenVINO 的职责 — 本地 AI 推理加速：**

- **OCR 推理加速** — V0.3 图片 OCR 使用 RapidOCR（ONNX 模型），在 Intel AI PC 上可通过 OpenVINO 切换为 NPU/GPU 异构加速
- **意图识别** — V1.0 Agent 大脑理解用户的自然语言请求（如"检查这个合同能不能外发"）
- **任务规划** — 判断需要调用哪些工具、按什么顺序执行
- **报告总结** — 将规则引擎的检测结果转化为自然语言的风险说明和处理建议

**规则引擎的职责 — 轻量化检测执行：**

- **敏感信息识别** — 正则 + 关键词匹配，高效、确定、无需模型推理
- **风险分级** — 基于命中类型的规则判定，高/中/低/提示四级分类
- **脱敏处理** — 对原始值进行格式化的掩码替换
- **报告骨架生成** — Markdown/JSON 结构化数据输出
...
用户自然语言请求
      ↓
[OpenVINO 本地模型] → 理解意图 → 规划任务 → 调用工具
      ↓
[规则引擎] → 文件解析 → 正则检测 → 风险分级 → 脱敏输出
      ↓
[OpenVINO 本地模型] → 解读结果 → 生成建议 → 自然语言回复
      ↓
用户获得：专业风险报告 + 场景化处理建议
```

这种架构的优势：
- **检测确定性高** — 规则引擎对标准格式（手机号、身份证号、API Key）识别率 100%，不受模型幻觉影响
- **推理成本低** — 规则检测零 GPU/NPU 开销，只有意图理解和报告总结需要模型
- **响应速度快** — 单文件扫描 < 1 秒，模型只在首尾参与，不参与中间检测过程
- **本地隐私安全** — 模型运行在本地（OpenVINO / Ollama），文件内容不离开设备

这是 AI PC 从"硬件概念"走向"生产力工具"的典型路径：**OpenVINO 驱动 35B 小模型做"大脑"，规则引擎做"双手"，模型负责理解和建议，规则负责精确执行。**
...
- ✅ **四级风险分级** + 自动脱敏 + 场景化建议
- ✅ **纯本地运行**，文件不出电脑，零 API 成本
- ✅ **Agent 可调用**，支持自然语言交互
- ✅ 符合 **ModelScope Skills** 开放标准

### 参赛标签

- **Skill 标签**: `AIPC` `privacy` `security` `local-ai` `OCR` `OpenVINO`
- **文章标签**: `Intel AI PC`
...
| 版本 | 目标 | 模型 vs 规则分工 |
|------|------|-----------------|
| **V0.1** | ✅ 单文件扫描 + 规则检测 | 规则主导，模型通过 API 辅助 |
| **V0.2** | ✅ docx/pdf/xlsx 解析 | 规则主导，模型通过 API 辅助 |
| **V0.3** | ✅ 图片 OCR 扫描（png/jpg/jpeg/bmp/tiff/webp） | OCR 模型提取文字 → 规则引擎检测；OpenVINO 加速预留 |
| **V0.4** | 多文件/文件夹批量扫描 | 规则批量执行，模型统一总结 |
| **V1.0** | OpenVINO 本地模型驱动 Agent 完整版 | **OpenVINO 本地推理**：意图识别 + 任务规划 + 工具调用 + 报告总结；规则引擎：具体检测执行 |
...
- **技术栈**: Python 3.8+（标准库 + python-docx / pypdf / openpyxl / rapidocr-onnxruntime）
- **License**: Apache-2.0
